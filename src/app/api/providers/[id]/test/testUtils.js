...
async function testOAuthConnection(connection, effectiveProxy = null) {
    ...
    try {
        const testUrl = config.buildUrl
            ? config.buildUrl(accessToken)
            : config.url;
        ...
        const accepted =
            ...
            (config.acceptStatuses &&
                config.acceptStatuses.includes(res.status));
    ...
    try {
        switch (connection.provider) {
            ...
            case "azure": {
                ...
                if (psd.organization)
                    headers["OpenAI-Organization"] = psd.organization;
                const res = await fetchWithConnectionProxy(
                    url,
                    {
                        method: "POST",
            ...
            case "perplexity-web": {
                ...
                if (
                    sessionToken.startsWith("__Secure-next-auth.session-token=")
                )
