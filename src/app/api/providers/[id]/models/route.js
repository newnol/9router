...
const appendCodexReviewModels = (models) =>
    models.flatMap((model) => {
        ...
        if (!id) return [];
        const name =
            model?.display_name || model?.displayName || model?.name || id;
        const normalized = { ...model, id, name };
        const isChatModel =
            (model?.type || "llm") !== "image" &&
    ...
    async (connection) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                console.log(
                    `${errorLabel} (falling back to static):`,
                    errorText,
                );
            }
        } catch (error) {
...
const PROVIDER_MODELS_CONFIG = {
    ...
    hyperbolic: createOpenAIModelsConfig(
    ...
    assemblyai: createOpenAIModelsConfig(
    ...
    kiro: {
        customResolver: async (connection) => {
            ...
            try {
                ...
                warning =
                    "Kiro returned no models; falling back to static catalog.";
            } catch (error) {
    ...
    "ollama-local": {
        customResolver: async (connection) => {
            ...
            if (!response.ok) {
                const errorText = await response.text();
                console.log(
                    "Error fetching models from ollama-local:",
                    errorText,
                );
                return {
